...
export function CustomerAccount() {
  ...
  const loadAccount=useCallback(async(accessToken:string,id:string,accountEmail="")=>{
    try {
      const [profileRows,orderRows]=await Promise.all([
        ...
        db(`orders?user_id=eq.${id}&select=id,order_number,currency,subtotal,shipping,tax,total,status,payment_status,tracking_number,shipping_address,created_at,order_items(id,product_name,quantity,unit_price,selected_size,selected_colour)&order=created_at.desc`,accessToken),
      ]);
  ...
  return <div className="customerDashboard">
    ...
    <main className="customerAccountMain"><a href="/" style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 16px",marginBottom:16,border:"1px solid #54233A",borderRadius:8,background:"#FFFDF7",color:"#54233A",fontSize:14,fontWeight:600,textDecoration:"none"}}><ShoppingBag size={16}/>Back to store</a><header className="accountMobileHead"><div className="customerAvatar small">{avatar}</div><div><small>MY KAOMA</small><strong>{profile.full_name||"Your account"}</strong></div></header><div className="accountMobileNav">{navItems.map(({id,label,icon:Icon})=><button key={id} className={section===id?"active":""} onClick={()=>setSection(id)}><Icon/>{label}</button>)}</div>{message&&<div className="accountMessage">{message}</div>}
      ...
      {section==="orders"&&<section className="accountSection"><AccountTitle eyebrow="ORDER HISTORY" title="My orders" text="Review every KAOMA purchase and its current status."/><div className="accountPanel orderHistory">{orders.length?orders.map(order=><OrderCard key={order.id} order={order} profile={profile} onTrack={()=>setSection("tracking")}/>):<EmptyOrders/>}</div><ReturnRequestPanel token={token} orders={orders}/></section>}
