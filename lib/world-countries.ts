export type WorldCountry = { code: string; name: string; dialCode: string };

// ISO 3166-1 destinations with international calling prefixes.
const rows = `AF|Afghanistan|+93
AL|Albania|+355
DZ|Algeria|+213
AD|Andorra|+376
AO|Angola|+244
...
export const worldCountries: WorldCountry[] = rows.split("\n").map((row) => {
  const [code, name, dialCode] = row.split("|");
